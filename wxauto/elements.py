...
class SelfMessage(Message):
    ...
    def parse(self):
        ...
        msglist = chatrecordwnd.ListControl()

        msgs = []
        seen_ids = set()  # 用于去重
        total_loaded = 0  # 上次获取到的消息数量
        no_new_count = 0  # 连续无新增次数计数器

        while True:
            items = msglist.GetChildren()
            new_items = []

            for msgitem in items:
                ...
                content = textcontrols[2].Name if len(textcontrols) > 2 else ''
                md5 = hashlib.md5(content.encode()).hexdigest()
                unique_key = f"{runtime_id}_{md5}"
                if unique_key not in seen_ids:
                    seen_ids.add(unique_key)
                    new_items.append(msgitem)

            # 处理新消息
            for msgitem in new_items:
                textcontrols = [i for i in GetAllControl(msgitem) if i.ControlTypeName == 'TextControl']
            ...
            if current_count == total_loaded:
                no_new_count += 1
                if no_new_count >= 3:  # 如果连续3次没有新消息，认为已经到底
                    break
            else:
                no_new_count = 0
            ...
            # 小幅度滚动
            msglist.WheelDown(wheelTimes=3, waitTime=0.5)
